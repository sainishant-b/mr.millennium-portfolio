...
const TETRIS_PIECES = [
  { shape: [[1, 1, 1, 1]], color: 'bg-accent' },
  { shape: [[1, 1], [1, 1]], color: 'bg-accent' },
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-accent' },
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-accent' },
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-accent' },
...
export default function TetrisLoading({
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(cell => ({ ...cell, color: 'bg-accent animate-pulse opacity-50' }))
          }
          return row
        })
        setTimeout(() => {
  ...
  return (
    <div>
      <div className="mb-4">
        <div className={`border-2 border-border bg-background ${config.padding} pixel-panel`}>
          {renderGrid()}
        </div>
      </div>
      {showLoadingText && (
        <div className="text-center">
          <p className="font-display text-[0.55rem] text-accent">{loadingText}</p>
        </div>
      )}
    </div>
  )
