...
interface ParticleTextEffectProps {
  words?: string[]
  /** Particle color — defaults to the theme accent red */
  particleColor?: { r: number; g: number; b: number }
  /** Font used to render text on the off-screen canvas */
  font?: string
  className?: string
}

const DEFAULT_WORDS = ["Mr.Millennium"]
// Theme accent: oklch(0.45 0.19 28.5) ≈ rgb(183, 14, 15)
const DEFAULT_COLOR = { r: 183, g: 14, b: 15 }

export function ParticleTextEffect({
  words = DEFAULT_WORDS,
  particleColor = DEFAULT_COLOR,
  font = 'bold 72px "Press Start 2P", monospace',
  className = "",
  ...
  const generateRandomPos = useCallback(
    ...
    const nextWord = useCallback(
      (word: string, canvas: HTMLCanvasElement) => {
        ...
        // Scale font size dynamically based on canvas width to avoid cutoffs
      ...
      // Use the fixed accent color with slight per-cycle variation for life
      const variation = () => Math.round((Math.random() - 0.5) * 30)
      const newColor = {
        r: Math.min(255, Math.max(0, particleColor.r + variation())),
        g: Math.min(255, Math.max(0, particleColor.g + variation())),
        b: Math.min(255, Math.max(0, particleColor.b + variation())),
      }
    ...
    [font, generateRandomPos, particleColor]
  )
      ...
      for (const entry of entries) {
        if (entry.contentBoxSize) {
          setDimensions({
            ...
            height: entry.contentRect.height > 0 ? entry.contentRect.height : 400
          })
        }
      }
    })
  ...
  }, [dimensions.width, dimensions.height, words, nextWord])
